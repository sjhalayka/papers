...
For instance, where $r_{r} = r_{e} \times 0.01$, it is found that gravitation can be repulsive at far distance.
...
For instance, the mean onset distance for Earth is $1.03$ astronomical units (AU).
...
\label{fig4}
  ...
  \caption{
A plot of the onset of repulsive gravitation mean distance $d$ based on field line count $n$, where $d \approx 0.01845 \times n^{0.723}$.
That is, the result forms a power law.
}
\end{figure}

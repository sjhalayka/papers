...
In this paper, we find a match between the numerical gravitation and the gravitational time dilation from Schwarzschild's general relativity \cite{hooft}.
...
\end{lstlisting}
The cosine weighting is given by:
\begin{lstlisting}
...
To contrast, the following code uses the Newtonian gravitation, where the random unit vector points in the same direction as the accompanying normal:
\begin{lstlisting}

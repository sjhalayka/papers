...
The entropy gained during the assimilation must be preserved in order to comply with the second law of thermodynamics (the change in entropy over the change in time must be non-negative):
\begin{equation}
dS/dt \geq 0.
\end{equation}
...
The time rate $dt$ in a gravitational well that surrounds a rotating, electrically-neutral (Kerr) black hole is:
\begin{equation}
...
\end{equation}
If $x = 0$, then the internal process occurs by chance, else the external process occurs.
...
The maximum entropy for a body is assumed to be the Bekenstein-Hawking entropy \cite{jacobson}, which is the black hole event horizon area divided by $4$:
\begin{equation}
S = 2 \pi M \left(M + \sqrt{M^2 - (J/M)^2}\right).
\end{equation}

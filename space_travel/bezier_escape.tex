...
For simplicity sake, each oscillator has a mass of $M_i = M/n$, and an angular momentum of $J_i \approx 0$.
...
To summarize each section separately:
\begin{enumerate}
\item Time dilation causes the irreversible disruption of internal process.
\item Entropy is the measure of external connectivity.
\item The cycle rate is an upper bound on how often internal process occurs.
\item Processes can be trained.
\item Black holes are perfect examples of external connectivity.
\item The maximum entropy is the Bekenstein-Hawking entropy.
\item The relativistic Doppler effect is good for calculating per-oscillator focus.

\end{enumerate}

To summarize altogether:

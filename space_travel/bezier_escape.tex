...
The maximum entropy for an accelerated black hole is assumed to be the Bekenstein-Hawking entropy \cite{jacobson}, which is the black hole event horizon area divided by $4$:
\begin{equation}
S_{BH} = 2 \pi M \left(M + \sqrt{M^2 - (J/M)^2}\right).
\end{equation}
The entropy is independent of the black hole's speed -- it's all external process, all the time, and it's independent of how focused the process is in one particular direction.
Otherwise, for non-black holes, it's assumed that the entropy of an accelerated test body can never reach $S_{BH}$, because the speed of light is not attainable -- the process can never be perfectly hyperfocused in one particular direction.

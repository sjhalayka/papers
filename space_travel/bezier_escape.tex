...
Where $n$ is the number of asymmetric oscillators comprising the non-black hole, the total dot product $y$ is:
\begin{equation}
y = \sum_{i = 1}^{n}  \sum_{j = i + 1}^{n} {\vec{A}}_i  \cdot {\vec{A}}_j.
\end{equation}
The 3D vectors ${\vec{A}}_i$ and ${\vec{A}}_j$ both have a length $\ell \in (0, 1]$, which represents the oscillator's degree of asymmetry.
The total number of dot product comparisons $z$ is:
\begin{equation}
z = \frac{n(n - 1)}{2}.
\end{equation}
The focus $w$ (anisotropy) is the normalized average dot product:
\begin{equation}
w = \frac{1 + {y}/{z}}{2} \in [0, 1),
\end{equation}
The entropy is:
\begin{equation}
S = w S_{BH} \in [0,  4\pi M^2).
\end{equation}

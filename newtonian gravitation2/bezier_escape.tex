...
For instance, consider a fair $W = 2^n$-sided die, where the probability of each face coming up eventually works out to be $p_i = 1/W$ (e.g. the faces are equiprobable).
...
\sum_{i = 1}^{W} p_i = 1.
\end{equation}
For equiprobable microstates, like in the case of the 8-bit integers for instance, where $p_i = 1/W$:
\begin{equation}
n = \frac{\log W}{\log 2},
\end{equation}
the number of equiprobable, distinct 8-bit integers is $2^8 = 256$, and so $n = 8$ and $W= 256$.
This is to say that if you understand the concept of the 8-bit integers (e.g. 256-sided die), then you also understand binary entropy in the case of equiprobable microstates.

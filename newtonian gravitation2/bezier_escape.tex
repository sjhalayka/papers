...
\title{Real dimension in the Newtonian simulation of disk-like zero pressure density systems}
...
For instance, consider a fair $W = 2^n$-sided die, where the probability of each face coming up eventually works out to be $p_i = 1/W$ (e.g. the faces are equiprobable because the die is fair).

Then we discuss the numerical solution of the equations for the Newtonian (e.g., Keplerian) orbit, and the Galactic flat rotation curve orbit.

Finally, we use an analytical approach to the solve the equations for the Galactic flat rotation curve orbit.
...
\sum_{i = 1}^{W} p_i = 1.
\end{equation}
For equiprobable microstates, like in the case of the 8-bit integers (e.g. fair 256-sided die) for instance, where $p_i = 1/W$:
\begin{equation}
n = \frac{\log W}{\log 2},
\end{equation}
the number of equiprobable, distinct 8-bit integers is $2^8 = 256$, and so $n = 8$ and $W= 256$.
This is to say that if you understand the concept of the 8-bit integers (e.g. 256-sided dice), then you also understand binary entropy in the case of equiprobable microstates.
...
Note that the calculation allows for non-equiprobable probabilities, and so it's a bit more general.
\begin{lstlisting}
#include <iostream>
#include <map>
#include <string>
...
\end{equation}
where $d = 3 - D$ stands for disk-like, and the fractal dimension of the gravitational field is:
\begin{equation}
...
Below is an analytical code that calculates the dimension $D$ for the Galactic orbit of the Solar System.

...
\begin{abstract}
In this paper, we investigate the fractional dimension of the gravitational field in the simulation of the non-relativistic gravitation of disk-like zero-pressure density systems (e.g. galaxies).
...
In this paper we show that the gravitational field can be made to be anisotropic, which strengthens the effect of the gravitational acceleration.
...
\end{equation}
The following equation for the gradient strength was found to be suitable:
\begin{equation}
g = -\gamma \pi \approx \frac{n}{2 R^D}.
\end{equation}
See Fig. 2.
Thus, the approximation of the Newtonian acceleration $a_{\textit{Newton}}$, where $D = 3$, is:
\begin{equation}
...
\centering
\label{fig2}
  \includegraphics[width = 7 in]{g_versus_g_.png}
  \caption{
Image showing the actual, numerical solution "g.txt" (purple) versus the approximation "g\_.txt" (green).
Of course, better, more complex approximations exist (e..g. a cumulative distribution function, $\tanh$).

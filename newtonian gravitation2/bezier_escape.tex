...
In this section, $D = (2, 3]$ stands for dimension, and $d = 3 - D = [0, 1)$ stands for disk-like.
...
\begin{equation}
a_{\textit{Newton}} = \frac{v_{\textit{Newton}}^2}{R} = \sqrt{\frac{g R^{(1 - d)} c \hbar \log 2}{2 k \pi M}} = \frac{GM}{R^2},
\end{equation}
...
int main(void)
{
	...
	double a_Newton = sqrt((g * G * c * hbar * log(2.0))/(2 * R * k * pi));
	// double a_Newton_ = G * M / (R * R);

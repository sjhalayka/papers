...
See Fig 1.
Note that the interaction strength is isotropic, and that the emitter is spherical (3D) in shape.

Similarly, when the photon emission is circular, the inverse normalized interaction strength is $315.259$.
See Fig 2.
Note that the interaction strength orthogonal to the circle's plane reduces as the photon emitter goes from spherical to circular, and that the emitter is circular (2D) in shape.

Finally, when the photon emission is beam-like, the inverse normalized interaction strength is $1$ (by definition).
See Fig 3.
Note that the interaction strength for all angles other than head-on reduces as the photon emitter goes from spherical to beam-like, and that the emitter is beam-like (1D) in shape.
...
Note that to emit photons in a spherical way, the emitter must first be spherical (3D) in shape.
}
\end{figure}
...
Note that to emit photons in a circular way, the emitter must first be circular (2D) in shape.
}
...
\frame{\includegraphics[width = 1.0 in]{beam_emitter.png}}
  \caption{
Blue beam-like emitter at $(0, 0, 0)$. 
Green absorber at $(0, 4, 0)$. 
Ray-sphere intersection location is coloured in red.
$100\%$ of the photons emitted are absorbed.
Note that to emit photons in a beam-ike way, the emitter must first be beam-like (1D) in shape.
}
\end{figure}

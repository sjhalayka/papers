...
\end{equation}
\begin{equation}
D = \frac{\log ( c^3 \frac{v_n}{v} ) } { \log(c) },
\end{equation}
\begin{equation}
v = v_n c^{3 - D},
\end{equation}
...
To compare, where $v = \sqrt{GM/r} = 149108$, it is found that $D = 3$ exactly (e.g. where Newton's isotropic theory of gravitation still closely applies).

Also to compare, where $D = 3$ is held constant, then the total mass required for a constant orbit speed is increased, including the so-called dark matter:
\begin{equation}

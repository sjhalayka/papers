...
\paragraph{Gravitational time dilation.} A static observer at fixed $(r,\theta)$ measures proper time at a rate, relative to the coordinate time $t$ of an observer at infinity, given by $d\tau_g/dt = \sqrt{-g_{tt}}$. Reading $g_{tt}$ off the metric,
\begin{equation}\label{eq:grav-dilation}
\frac{d\tau_g}{dt} = \sqrt{-g_{tt}} = \sqrt{\,1 - \frac{2Mr}{\Sigma}\,} = \sqrt{\frac{r^2 - 2Mr + a^2\cos^2\theta}{r^2 + a^2\cos^2\theta}}.
\end{equation}
...
For a body with combined proper-time rate $\eta \in [0,1]$, the \emph{exogeneity} $e_x$ is any quantity satisfying

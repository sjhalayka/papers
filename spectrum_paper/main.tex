...
Throughout this paper we generally use \emph{Planck units}, in which the reduced Planck constant $\hbar$, the speed of light in vacuum $c$, the gravitational constant $G$, and the Boltzmann constant $k_B$ are all set equal to unity:
\begin{equation}
\hbar = c = G = k_B = 1.
\end{equation}
...
\centering
  ...
  \caption{
A sample spectrograph of the visible wavelengths, from the Sun, with Fraunhofer lines. Each labeled trough is an absorption line whose equivalent width \eqref{eq:equivalent-width} encodes the optical depth of the responsible constituents in the solar atmosphere.
}
\label{fig2}
\end{figure}
...
It is instructive to compare with the prediction of Newtonian gravity applied to a corpuscle moving at the speed of light in vacuum. Treating the photon as a fast test particle on a hyperbolic orbit and computing the transverse impulse, one finds the Newtonian deflection
\begin{equation}
\alpha_{\mathrm{N}} = \frac{2M}{b},
\end{equation}
exactly half of the relativistic result. 
Inserting the standard values into Eq.~\eqref{eq:deflection} yields
\begin{equation}
\alpha = \frac{4 G M_\odot}{c^2 R_\odot} \approx 8.49\times 10^{-6}\ \mathrm{radians} \approx 1.75\ \text{arcseconds},
\end{equation}
...
- \frac{\Sigma}{\Delta}\left(\dot v^r\right)^2
- \Sigma\left(\dot v^\theta\right)^2
- \frac{A\sin^2\theta}{\Sigma}\left(\dot v^\phi - \frac{2Mar}{A} \right)^2},
\end{equation}
where the velocity components are given in spherical coordinates (e.g. $\dot v^r = dr/dt$, $\dot v^\theta = d\theta/dt$, $\dot v^\phi = d\phi/dt$).
...
\begin{corollary}
At a black hole horizon or at the speed of light in vacuum, the endogeneity vanishes: the body has no internal process and is, in this precise sense, a perfect black body.
\end{corollary}
...
\section{Converting from Cartesian to spherical velocity}
Given the Cartesian \emph{velocity} $(\dot v^x,\dot v^y,\dot v^z)$, where a dot
denotes $d/dt$, we want the corresponding spherical velocity
$(\dot v^r,\dot v^\theta,\dot v^\phi)$.
Here, $r_{bh}, \theta_{bh}, \phi_{bh}$ describe the observer's position with respect to the black hole:
\begin{equation}
\begin{pmatrix} \dot v^r \\[2pt] \dot v^\theta \\[2pt] \dot v^\phi \end{pmatrix}
=
J
\begin{pmatrix} \dot v^x \\[2pt] \dot v^y \\[2pt] \dot v^z \end{pmatrix},
\qquad
J =
\begin{pmatrix}
...
\end{pmatrix} =
\begin{pmatrix}
\sin\theta_{bh}\cos\phi_{bh} & \sin\theta_{bh}\sin\phi_{bh} & \cos\theta_{bh} \\ \\
\dfrac{\cos\theta_{bh}\cos\phi_{bh}}{r_{bh}} & \dfrac{\cos\theta_{bh}\sin\phi_{bh}}{r_{bh}} & -\dfrac{\sin\theta_{bh}}{r_{bh}} \\ \\
-\dfrac{\sin\phi_{bh}}{r_{bh}\sin\theta_{bh}} & \dfrac{\cos\phi_{bh}}{r_{bh}\sin\theta_{bh}} & 0 \\
\end{pmatrix},
\end{equation}
where $J$ is the Jacobian matrix that does the conversion.
This produces the velocity needed for Eq.~\eqref{eq:sigma}.

Note that the Cartesian speed $s = \sqrt{\dot v^x \dot v^x + \dot v^y \dot v^y + \dot v^z \dot v^z}$ must be less than the speed of light in vacuum (e.g. $s < 1$) for a massive body.
...
Entropy can be taken to describe the level of intraconnectivity of a mass \cite{qg}.

For everyday materials at rest on the surface of the Earth, entropy is proportional to mass
\begin{equation}
S \propto M.
\end{equation}
The number of connections is like $6 N$ (e.g. up, down, left, right, forward, backward), where $N$ is the number of oscillators.

For highly gravitationally time dilated materials, entropy is proportional to mass squared
\begin{equation}
S \propto M^2.
\end{equation}
The number of connections is like $(N^2 - N)/2$.

For highly kinematically time dilated materials, entropy is next to zero
\begin{equation}
S \approx 0.
\end{equation}
...
\bibitem{qg} Konopka, et al. Quantum graphity. (2006)

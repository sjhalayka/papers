...
For a perfect sphere, the interaction strength is like $1$ with a long-range falloff proportional to $x = 1/r^2$.
For a perfect disk, the interaction strength increases by a factor of $c$ with a long-range falloff proportional to $x = 1/r$, and for a perfect filament it increases by a factor of $c^2$ with a long-range falloff proportional to $x = 1$ (e.g. no falloff).
...
In general, where $ 2 < D < 3$, the value of the dimension will actually be a little bit greater than that given by Eq. 10, since the falloff exponent $x$ is $(1/r) < x < (1/r^2)$.
...
%Where $D = 3$ is held constant, it is found that the total mass required for a constant orbit speed is increased, including the so-called dark matter:
%\begin{equation}
...
%This view that $M_{dark} > 0$ and the view that $D < 3$ are both correct at the same time -- they are complimentary ways of describing the same thing.

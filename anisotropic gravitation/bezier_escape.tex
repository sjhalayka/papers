...
Also to compare with the past literature, where $D = 3$ is held constant, it is found that the total mass required for a constant orbit speed is increased, including the so-called dark matter:
\begin{equation}
...
\end{equation}
That is, the condensation of the gravitons produces a measurable amount of dark matter.
This view that $M_{dark} > 0$ and the view that $D < 3$ are both correct -- they are complimentary ways of looking at the same thing.
...
If dark matter is a condensate of {\textit{gravitating} gravitons, then it is like the ants and their attractive, ultimately self-reinforcing pheromone trails -- that is, complex, emergent structure occurs at the Galactic scale and larger, where gravitation is practically the only binding force.
It's impossible to deny: the simulated Universe looks very much like ant trails, biological neural networks, and computer networks  -- the similarity is {\textit{not}} coincidental.
The rich get richer, so to speak \cite{mm}, when it comes to forming a scale-free network in the cosmos, just like it does with ant colonies, brains, and the World Wide Web.
...
It seems that the computational efficiency of the Universe is increasing over time in general, because of the $2$nd law of thermodynamics, thus causing this time contraction; this antithesis of gravity.

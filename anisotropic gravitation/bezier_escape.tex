...
\item The deflection of a relativistic mass by the Sun
...
\section{On the deflection of a relativistic mass by the Sun}

It is known that the deflection of light occurs due to the spacetime curvature caused by the Sun.
The deflection angle is
\begin{equation}
\delta = \frac{4GM}{c^2 r},
\end{equation}
where $r = 6.96 \times 10^8$ is the closest distance (e.g. the Sun's radius).
This is twice the amount obtained using Newtonian gravitation alone.
It is twice as much because light has no internal process to overcome.
Likewise, for a relativistic mass such as a neutrino, where $v \approx c$, there is practically no internal process to overcome, and so the strength of gravitation doubles.
In essence, $H = 2G = 1.33 \times 10^{-10}$ is the fundamental constant, not $G$, and it is seen that many equations simplify because of it.
For example, the Einstein field equations, which are used to create the Schwarzschild solution discussed in the previous section, are:
\begin{equation}
R_{\mu\nu} - \frac{1}{2} R g_{\mu\nu} + \Lambda g_{\mu\nu} = \kappa T_{\mu\nu},
\end{equation}
where $\kappa$ is
\begin{equation}
\kappa = \frac{4\pi H}{c^4}.
\end{equation}
...
v = v_n c^{3 - D},
\end{equation}
where $M = 1 \times 10^{41}$ is the mass of the Milky Way's bulge, $v = 220000$ is the observed circular orbit speed, and the orbit radius is $r = 3 \times 10^{20}$ metres (e.g. roughly $10$ kiloparsecs).
In general, where $D < 3$, the value of the dimension will actually be a little bit greater than that given by Eq. 10, since the falloff exponent is $x < 2$ and the strength of the gravitation itself is increased as well.
...
See Fig. 1 for C++ code for this section. 

%It is worth noting that the dimension $D$ divided by
%\begin{equation}
%\alpha = \frac{r_b}{r},
%\end{equation}
%where $r_b = 6.1495 \times 10^{19}$ metres is the bulge radius, plots a straight line.
%See Figs. 1 and 2.
%This goes to show that the dimension can be expressed either algebraically or geometrically.
...
%\begin{figure} 
%\centering
%  \includegraphics[width = 6 in]{dimension_graph.png}
%  \caption{ Graph where x axis is distance $r$ and y axis is dimension $D$.}
%\end{figure}


%\begin{figure} 
%\centering
%  \includegraphics[width = 6 in]{dimension_div_x.png}
%  \caption{  Graph where x axis is $r$ and y axis is $D / \alpha$, where $\alpha = r_b/r$.}
%\end{figure}

...
See Figs. 1 and 2 for an analytical solution for anisotropic gravitation.
...
For instance, Fig. 3 shows the intersection count (e.g. field strength) as the dimension of the gravitational field goes from $D = 2.001$ to $D = 3$. 
Figs. 4 through 6 show an example gravitational field that goes from $D = 3.0$ to $D = 2.5$ to $D = 2.001$.
Finally, Figs. 7 through 9 show that there are fractal patterns to be found when analyzing the data.
...
\centering
  ...
  \caption{
Here $D = 2.001$.
The $x$ axis is distance $r$, and the $y$ axis is intersection count times inverse falloff $f_i = r^1$.
...
\centering
  ...
  \caption{
Here $D = 2.5$.
The $x$ axis is distance $r$, and the $y$ axis is intersection count times inverse falloff $f_i = r^{1.5}$.
...
\centering
  ...
  \caption{
Here $D = 3$.
The $x$ axis is distance $r$, and the $y$ axis is intersection count times inverse falloff $f_i = r^2$.
Note that the output appears to be fractal in nature -- non-differentiable anyway.
And so, it appears that the more fractional the dimension, the more differentiable the result.
Here we define this fractionality as $\alpha = 1 - 2 (1/2 - (D\;\textrm{fmod}\;1.0))$, where $\textrm{fmod}$ is the floating-point (e.g. real) modulo operator.
}
\end{figure}

...
\item The dark matter problem: why is gravitation stronger than that predicted by general relativity in zero pressure density dusts such as the Galactic disc?
...
\item Gravitationally-bound, zero pressure density dusts, such as the Galactic disc, have fractional dimension. 
As the dimension reduces, the strength of the gravitation increases -- the dimension reduces as the shape of the Milky Way goes from spherical to disc-like with distance from the Galactic centre.
...
\section{Analytical solution for anisotropic gravitation}
...
In the case of gravitation, the transition from broadcast transmission to directed transmission occurs as dark matter is factored in (e.g. where $D < 3$) -- see Figs. 1 and 2 for a simplified analytical solution for anisotropic gravitation in the Milky Way.
...
\item Why is gravitation stronger than that predicted by general relativity in zero pressure density dusts such as the Galactic disc? 
...
\item Can a human-scale object have zero pressure density internally? 
...
A copy of the C++ code for Figs. 3 through 8 can be obtained from:
...
Figs. 7 and 8 show an analysis of the data -- the result is that $GM c^{3 - D}$ is a constant.
...
The oscillators are placed pseudorandomly on a 2-sphere, and the normals are calculated using the same sphere.
...
The oscillators are placed on an oblate (e.g. flattened) ellipsoid, and the normals are calculated by using the dual prolate (e.g. tall) ellipsoid.

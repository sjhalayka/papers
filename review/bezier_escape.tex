...
Fig 7. shows an analysis of the data -- the result is that $GM$ is a constant.
...
Finally, Figs. 7 through 9 show that there are fractal-like patterns to be found when analyzing the data -- here we define the fractionality of $D$ as it goes from $D = 3.0$ to $D = 2.001$ as
\begin{equation}
...
Note that the output appears to be non-differentiable, but non-fractal.
}
\end{figure}
...
Note that the output appears to be differentiable, but non-fractal.
}
\end{figure}
...
\centering
  ...
  \caption{
The $x$ axis is distance $r$, and the $y$ axis (log scaled) are the intersection count gradient times the inverse falloffs $r^2$, $r^{2.5}$, and $r^3$.
}
\end{figure}
